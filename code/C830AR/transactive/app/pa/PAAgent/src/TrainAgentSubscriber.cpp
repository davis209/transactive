 ...
 * in any form.
 *
 * Source:   $File: //depot/C830AR_TIP/C830AR/transactive/app/pa/PAAgent/src/TrainAgentSubscriber.cpp $
 * @author:  Ripple
 * @version: $Revision: #3 $
 *
...
#include "TrainAgentSubscriber.h"
...
namespace TA_IRS_App
{
        ...
        m_iscsLibraryVersion(0)
    {
        ...
        PasHelpers::instance().closeTrainLibraryVersionAlarms();
    ...
    void TrainAgentSubscriber::onGlobalTrainVersionChanged()
    {
        ...
        PasHelpers::instance().closeTrainLibraryVersionAlarmOnVersionChange(m_iscsLibraryVersion);
    ...
    void TrainAgentSubscriber::processTrainLibraryInfo(const TA_IRS_Bus::PaTypes::TrainLibraryInfo& event)
    {
        ...
        LOG_INFO("Processing Live Announcement info Train=%d Version=%d", static_cast<int>(event.trainId), static_cast<int>(event.libraryVersion));

        // compare the versions
        if (event.libraryVersion != m_iscsLibraryVersion)
        {
            // raise an alarm
            PasHelpers::instance().raiseTrainLibraryVersionAlarm(event.trainId, event.libraryVersion, m_iscsLibraryVersion);
        }
        else
        {
            PasHelpers::instance().closeTrainLibraryVersionAlarm(event.trainId);
        }
